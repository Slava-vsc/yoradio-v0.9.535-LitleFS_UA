...
//  #define BATTERY_ON      config.store.batteryon			// Статус батарейки)
  ...
  #define ProcX     TFT_FRAMEWDT+6		// X coordinate for percent (Координата X для процентов заряда)
...
const ScrollConfig weatherConf    PROGMEM = {{ 0, 53, 1, WA_LEFT }, 250, true, DSP_WIDTH-6*4, 0, 2, 25 };	// Weather (погода)
...
#ifdef CPU_LOAD
    const WidgetConfig  cpuConf   PROGMEM = { 0, 53, 1, WA_LEFT };
#endif
