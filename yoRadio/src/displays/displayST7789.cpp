...
void DspCore::flip(){
  ...
  if(ROTATE_90) setRotation(config.store.flipscreen?0:2); else setRotation(config.store.flipscreen?3:1);
#endif
#if DSP_MODEL==DSP_ST7789_170 || DSP_MODEL==DSP_ST7789_76
  setRotation(config.store.flipscreen?3:1);
#endif
#if DSP_MODEL==DSP_ST7789_240
if(ROTATE_90) setRotation(config.store.flipscreen?3:1); else setRotation(config.store.flipscreen?2:0);
#endif
}
void DspCore::invert(){ invertDisplay((DSP_MODEL==DSP_ST7789_170)?!config.store.invertdisplay:config.store.invertdisplay); }
