...
#include <freertos/FreeRTOS.h>	   // SPIFFS -> LittleFS
#include <freertos/task.h>		   // SPIFFS -> LittleFS
...
void u8fix(char *src){   // Якщо немає останнього байта останнього багатобайтового символу (з діакритичним знаком), початок обрізається.
...
bool Config::_isFSempty() {
  const char* reqiredFiles[] = {"dragpl.js.gz","ir.css.gz","irrecord.html.gz","ir.js.gz","logo.svg.gz","options.html.gz","player.html.gz","script.js.gz",
        "search.html.gz", "style.css.gz","updform.html.gz","theme.css"};
  const uint8_t reqiredFilesSize = 12;
  char fullpath[64];   // Безпечний розмір для LittleFS
  if(LittleFS.exists("/www/settings.html")) LittleFS.remove("/www/settings.html");   // SPIFFS -> LittleFS
  if(LittleFS.exists("/www/update.html")) LittleFS.remove("/www/update.html");   // SPIFFS -> LittleFS
  if(LittleFS.exists("/www/index.html")) LittleFS.remove("/www/index.html");   // SPIFFS -> LittleFS
  if(LittleFS.exists("/www/ir.html")) LittleFS.remove("/www/ir.html");   // SPIFFS -> LittleFS
  if(LittleFS.exists("/www/elogo.png")) LittleFS.remove("/www/elogo.png");   // SPIFFS -> LittleFS
  if(LittleFS.exists("/www/elogo84.png")) LittleFS.remove("/www/elogo84.png");   // SPIFFS -> LittleFS
  for (uint8_t i=0; i<reqiredFilesSize; i++){
//    sprintf(fullpath, "/www/%s", reqiredFiles[i]);
// Використовуємо snprintf, яка жорстко контролює розмір буфера (64 байти)
    snprintf(fullpath, sizeof(fullpath), "/www/%s", reqiredFiles[i]);
    if(!LittleFS.exists(fullpath)) {   // SPIFFS -> LittleFS
...
void Config::init() {
  ...
  if (!LittleFS.begin(true)) {   // SPIFFS -> LittleFS
    Serial.println("##[ERROR]#\tLittleFS Mount Failed");   // SPIFFS -> LittleFS
    return;
  }
  BOOTLOG("LittleFS mounted");   // SPIFFS -> LittleFS
...
// Перевірка на порожнечу (метод _isFSempty теж повинен використовувати LittleFS всередині)
  emptyFS = _isFSempty();
  if(emptyFS) BOOTLOG("LittleFS is empty!");   // SPIFFS -> LittleFS
  ssidsCount = 0;
// Робота з вказівниками на файлову систему
  #ifdef USE_SD
// Якщо режим SD – використовуємо sdman, інакше – адреса об'єкта LittleFS
  _SDplaylistFS = getMode()==PM_SDCARD?&sdman:(true?&LittleFS:_SDplaylistFS);   // SPIFFS -> LittleFS
  #else
  _SDplaylistFS = &LittleFS;   // SPIFFS -> LittleFS
  #endif
  _bootDone=false;
  setTimeConf();
}
...
void Config::_setupVersion(){
  ...
  switch(currentVersion){
    ...
    case 4:
      ...
      saveValue(&store.namestrim, true);     // Збереження значення за промовчанням в енергонезалежну пам'ять
...
bool Config::littlefsCleanup(){   // SPIFFS -> LittleFS
// Перевіряємо існування файлів перед видаленням (використовуємо LittleFS)
  bool ret = (LittleFS.exists(PLAYLIST_SD_PATH)) || (LittleFS.exists(INDEX_SD_PATH)) || (LittleFS.exists(INDEX_PATH));   // SPIFFS -> LittleFS
  if(LittleFS.exists(PLAYLIST_SD_PATH)) LittleFS.remove(PLAYLIST_SD_PATH);   // SPIFFS -> LittleFS
  if(LittleFS.exists(INDEX_SD_PATH)) LittleFS.remove(INDEX_SD_PATH);   // SPIFFS -> LittleFS
  if(LittleFS.exists(INDEX_PATH)) LittleFS.remove(INDEX_PATH);   // SPIFFS -> LittleFS
  return ret;
}
...
void Config::_initHW(){
  ...
  // Додаємо примусову ініціалізацію MUTE_PIN
  #ifdef MUTE_PIN
    if (MUTE_PIN != 255) {
      pinMode(MUTE_PIN, OUTPUT);
      // Встановлюємо початковий рівень (щоб було клацання при включенні)
...
void Config::resetSystem(const char *val, uint8_t clientId){
  if (strcmp(val, "system") == 0) {
    ...
    saveValue(&store.telnet, true);
    saveValue(&store.watchdog, true);
    saveValue(&store.namestrim, true);
  ...
  if (strcmp(val, "screen") == 0) {
    ...
    saveValue(&store.screensaverPlayingTimeout, (uint16_t)20);
    saveValue(&store.screensaverPlayingBlank, false);
    saveValue(&store.faderon, false);						// ******************************************
    saveValue(&store.fadermin, (uint8_t)2);				// ******************************************
    saveValue(&store.fadertimeout, (uint16_t)60);			// ******************************************
    display.putRequest(NEWMODE, CLEAR); display.putRequest(NEWMODE, PLAYER);
  ...
  if (strcmp(val, "timezone") == 0) {
    ...
    saveValue(store.sntp1, "ntp1.ntp-servers.net", 35, false);
  ...
  if (strcmp(val, "weather") == 0) {
    ...
    saveValue(&store.grndlevel, (uint16_t)GRND_HEIGHT, false);	// ******************************************
...
void Config::setDefaults() {
  ...
  store.brightness=100;
  store.contrast=55;
  strlcpy(store.sntp1,"ntp1.ntp-servers.net", 35);
  ...
  store.screensaverPlayingTimeout = 20;
  ...
  store.vufadespeed = 8;
  #ifdef GRND_HEIGHT
    store.grndlevel = GRND_HEIGHT;
  #else
    store.grndlevel = 0;
  #endif

  #ifndef BATTERY_ON
    store.batteryon = false;
  #else
    store.batteryon = true;
  #endif
  #ifdef HIDE_VOLT
    store.voltageon = false;
  #else
    store.voltageon = true;
  #endif
  #ifdef DELTA_BAT
    store.batoffset = DELTA_BAT;
  #else
    store.batoffset = 0.0f;
  #endif

  #if (BRIGHTNESS_PIN!=255)
    #ifdef DOWN_LEVEL
      store.faderon = true;
      store.fadermin = DOWN_LEVEL;
    #else
      store.fadermin = 2;
    #endif
    #ifdef DOWN_INTERVAL
      store.faderon = true;
      store.fadertimeout = DOWN_INTERVAL;
    #else
      store.fadertimeout = 60;
    #endif
  #else
    store.faderon = false;
    store.fadermin = 0;
    store.fadertimeout = 0;
  #endif
 								// **************************************
...
void Config::saveVolume(){
  saveValue(&store.volume, store.volume, false, true);   // зміна гучності не записується на флешпам'ять, тільки при збереженні налаштувань
}
...
void Config::setVuFadespeed(uint8_t vufs) { 					// *****************************************
    saveValue(&store.vufadespeed, vufs); 
}

void Config::setGRNDLevel(uint16_t grndl) { 					// *****************************************
    saveValue(&store.grndlevel, grndl, true); 
}
...
// Відкриваємо файл плейлиста для читання з LittleFS
  File playlist = LittleFS.open(PLAYLIST_PATH, "r");   // SPIFFS -> LittleFS
  if (!playlist) {
    return;
  }
  int sOvol;
// Створюємо файл індексу для запису в LittleFS
  File index = LittleFS.open(INDEX_PATH, "w");   // SPIFFS -> LittleFS
...
void Config::initPlaylist() {
  if (!LittleFS.exists(INDEX_PATH)) indexPlaylist();   // SPIFFS -> LittleFS
...
bool Config::loadStation(uint16_t ls) {
  ...
  if (cs == 0) {
    ...
    strncpy(station.name, "Виберіть радіо-станцію", BUFLEN);
    station.ovol = 0;
    return false;
  }
...
bool Config::saveWifiFromNextion(const char* post){
  File file = LittleFS.open(SSIDS_PATH, "w");   // SPIFFS -> LittleFS
  if (!file) {
    return false;
  } else {
    file.print(post);
...
bool Config::saveWifi() {
  ...
  LittleFS.remove(SSIDS_PATH);   // SPIFFS -> LittleFS
  LittleFS.rename(TMP_PATH, SSIDS_PATH);   // SPIFFS -> LittleFS
  ESP.restart();
  return true;
}
...
bool Config::initNetwork() {
  File file = LittleFS.open(SSIDS_PATH, "r");   // SPIFFS -> LittleFS
...
//      BOOTLOG("display:\tmodel: %d, (CS-%d, DC-%d, RST-%d, SPI-%s)", DSP_MODEL, TFT_CS, TFT_DC, TFT_RST, DSP_SSPI?"SSPI, MOSI-35, SCLK-36":"FSPI, MOSI-11, SCLK-12");
      BOOTLOG("display:\tmodel: %d, (CS-%d, DC-%d, RST-%d, SPI-%s)", DSP_MODEL, TFT_CS, TFT_DC, TFT_RST,  "FSPI, MOSI-11, SCLK-12");
    #else
      BOOTLOG("display:\tmodel: %d, (CS-%d, DC-%d, RST-%d, SPI-%s)", DSP_MODEL, TFT_CS, TFT_DC, TFT_RST, DSP_HSPI?"HSPI":"VSPI, MOSI-11, SCLK-12");
    #endif
  ...
  if(VS1053_CS==255) {
    BOOTLOG("audio:\t\t%s (DOUT-%d, BCLK-%d, LRC-%d)\tVersion %s", "I2S", I2S_DOUT, I2S_BCLK, I2S_LRC, I2SVER);
  }else{
    #ifdef ARDUINO_ESP32S3_DEV
//        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s)  Version %s", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, VS_SSPI?"SubSPI":"FSPI", I2SVER);
        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s)  Version %s", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, "FSPI", I2SVER);
//        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s, MOSI-%d, MISO-%d, SCK-%d)", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, VS_SSPI?"SubSPI":"FSPI", mosi_pin, miso_pin, sclk_pin);
    #else
        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s)  Version %s", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, VS_HSPI?"HSPI":"VSPI", I2SVER);
//        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s, MOSI-%d, MISO-%d, SCK-%d)", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, VS_HSPI?"HSPI":"VSPI", mosi_pin, miso_pin, sclk_pin);
  ...
  BOOTLOG("battery:\t%s", (config.store.batteryon)?"ON":"OFF");
  BOOTLOG("------------------------------------------------");
}
