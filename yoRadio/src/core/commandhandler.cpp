...
bool CommandHandler::exec(const char *command, const char *value, uint8_t cid) {
  ...
  if (strEquals(command, "clearlittlefs")){ config.littlefsCleanup(); config.saveValue(&config.store.play_mode, static_cast<uint8_t>(PM_WEB)); return true; }   // SPIFFS -> LittleFS
  /*********************************************/
  /****************** WEBSOCKET ****************/
  /*********************************************/
  ...
  if (strEquals(command, "screensaverplayingblank"))  { config.setScreensaverPlayingBlank(static_cast<bool>(atoi(value))); return true; }
    ...
    if (strEquals(command, "namestrim")) {
        config.saveValue(&config.store.namestrim, static_cast<bool>(atoi(value)));
        return true;
    }
  ...
  if (strEquals(command, "grndl"))    { config.setGRNDLevel(static_cast<uint16_t>(atoi(value))); return true; }	// **********************************
  ...
  if (strEquals(command, "format"))  { LittleFS.format(); ESP.restart(); return true; }   // SPIFFS -> LittleFS
