...
struct config_t
{
  ...
  bool   faderon;					// ******************************************
  uint8_t   fadermin;					// ******************************************
  uint16_t   fadertimeout;				// ******************************************
  uint16_t  abuff;
  bool      telnet;
  bool      watchdog;
  bool      namestrim;    // Додаткова змінна. ВКЛ/ОТКЛ "NAME_STRIM"
...
class Config {
  ...
  public:
    ...
    void setVuFadespeed(uint8_t vufs); 					// *******************************************
    void setGRNDLevel(uint16_t grndl); 					// *******************************************
    bool littlefsCleanup();   // SPIFFS -> LittleFS
    void waitConnection();
