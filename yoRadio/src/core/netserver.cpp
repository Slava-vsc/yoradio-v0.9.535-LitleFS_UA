#include "options.h"
#include "Arduino.h"
#include <LittleFS.h>   // SPIFFS -> LittleFS
#include <Update.h>
#include "config.h"
#include "netserver.h"
#include "player.h"
...
extern void updateFaderState(); 			// For Plugin BacklightDown
...
bool NetServer::begin(bool quiet) {
  ...
  webserver.serveStatic("/", LittleFS, "/www/").setCacheControl("max-age=31536000");   // SPIFFS -> LittleFS
#ifdef CORS_DEBUG
...
size_t NetServer::chunkedHtmlPageCallback(uint8_t* buffer, size_t maxLen, size_t index){
  ...
  if(sdpl){
    ...
    requiredfile = LittleFS.open(netserver.chunkedPathBuffer, "r");   // SPIFFS -> LittleFS
  }
...
void NetServer::processQueue(){
  ...
  if(xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS)){
    ...
    switch (request.type) {
      ...
      case GETACTIVE: {
          ...
          if (network.status == CONNECTED) {
            ...
            if (!SHOW_WEATHER || dbgact) {
                ...
                nxtn=true;   // Тепер виконається суворо всередині умови if
            }
          #endif
      ...
      case GETSYSTEM:     sprintf (wsBuf, "{\"sst\":%d,\"aif\":%d,\"vu\":%d,\"softr\":%d,\"vut\":%d,\"vufs\":%d,\"mdns\":\"%s\",\"batt\":%d,\"voltg\":%d,\"bofs\":%.3f,\"ipaddr\":\"%s\", \"abuff\": %d, \"telnet\": %d, \"watchdog\": %d, "
										  "\"namestrim\": %d }",  // "namestrim"
                                  config.store.smartstart != 2, 
                                  config.store.audioinfo, 
                                  config.store.vumeter, 
                                  config.store.softapdelay,
                                  config.vuThreshold,
                                  config.store.vufadespeed, 				// **************************************
                                  config.store.mdnsname,
                                  config.store.batteryon, 				// **************************************
                                  config.store.voltageon, 				// **************************************
                                  config.store.batoffset, 				// **************************************
                                  config.ipToStr(WiFi.localIP()),
                                  config.store.abuff,
                                  config.store.telnet,
                                  config.store.watchdog,
                                  config.store.namestrim);
                                  break;
      case GETSCREEN:     sprintf (wsBuf, "{\"flip\":%d,\"inv\":%d,\"nump\":%d,\"tsf\":%d,\"tsd\":%d,\"dspon\":%d,\"br\":%d,\"con\":%d,\"scre\":%d,\"scrt\":%d,\"scrb\":%d,\"scrpe\":%d,\"scrpt\":%d,\"scrpb\":%d,\"fadr\":%d,\"fmin\":%d,\"fadt\":%d}", 
                                  ...
                                  config.store.faderon, 				// **************************************
                                  config.store.fadermin, 				// **************************************
                                  config.store.fadertimeout); 			// **************************************
                                  break;
      case GETTIMEZONE:   sprintf (wsBuf, "{\"tzh\":%d,\"tzm\":%d,\"sntp1\":\"%s\",\"sntp2\":\"%s\", \"timeint\":%d,\"timeintrtc\":%d}",
                                  ...
                                  break;
      case GETWEATHER:    sprintf (wsBuf, "{\"wen\":%d,\"wlat\":\"%s\",\"wlon\":\"%s\",\"grndl\":%d,\"wkey\":\"%s\",\"wint\":%d}",
                                  config.store.showweather, 
                                  config.store.weatherlat, 
                                  config.store.weatherlon, 
                                  config.store.grndlevel, 					// *******************************
...
void NetServer::onWsMessage(void *arg, uint8_t *data, size_t len, uint8_t clientId) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    if (config.parseWsCommand((const char*)data, _wscmd, _wsval, 65)) {
      ...
      if (strcmp(_wscmd, "batteryon") == 0) { 							// On/Off Battery
        config.saveValue(&config.store.batteryon, (bool)atoi(_wsval), true);
//Serial.printf("###################config Battery Status: %s\n", (config.store.batteryon)?"ON":"OFF");			// Контроль
        displayMode_e m = display.mode();
        display.putRequest(NEWMODE, CLEAR);
        display.putRequest(NEWMODE, m);
        return;
      }
      if (strcmp(_wscmd, "voltageon") == 0) { 							// On/Of Voltage
        config.saveValue(&config.store.voltageon, (bool)atoi(_wsval), true);
//Serial.printf("###################config Battery Voltage: %s\n", (config.store.voltageon)?"ON":"OFF");			// Контроль
        displayMode_e m = display.mode();
        display.putRequest(NEWMODE, CLEAR);
        display.putRequest(NEWMODE, m);
        return;
      }
      if (strcmp(_wscmd, "batoffset") == 0) {
        config.saveValue(&config.store.batoffset, (float)atof(_wsval), true); 		// Set Voltage offset
//Serial.printf("######################### Battery offset: %.3f\n", (float)atof(_wsval));						// Контроль
        return;
      }
      ...
      if (strcmp(_wscmd, "vufadespeed") == 0) { 					// Set VU faderspeed
        config.saveValue(&config.store.vufadespeed, (uint8_t)atoi(_wsval), true);
        netserver.requestOnChange(GETSYSTEM, clientId);
        return;
      }

      if (strcmp(_wscmd, "faderon") == 0) { 						// On/Off Fader
        #if (BRIGHTNESS_PIN != 255)
          config.saveValue(&config.store.faderon, (bool)atoi(_wsval), true);
//Serial.printf("###################config Fader Status: %s\n", (config.store.faderon)?"ON":"OFF");		// Контроль
          updateFaderState();
        #endif
        return;
      }
      if (strcmp(_wscmd, "fadermin") == 0) { 						// Set Fader minimum
        #if (BRIGHTNESS_PIN != 255)
          config.saveValue(&config.store.fadermin, (uint8_t)atoi(_wsval), true);
//Serial.printf("######################### Set Fader min: %d\n", config.store.fadermin);			// Контроль
          updateFaderState();
        #endif
        return;
      }
      if (strcmp(_wscmd, "fadertimeout") == 0) { 					// Set Fader timeout
        #if (BRIGHTNESS_PIN != 255)
          config.saveValue(&config.store.fadertimeout, (uint16_t)atoi(_wsval), true);
//Serial.printf("######################### Set Fader Time: %d\n", config.store.fadertimeout);		// Контроль
          updateFaderState();
        #endif
        return;
      }

      if (strcmp(_wscmd, "grndl") == 0) { 							 // Set Ground Level
        config.saveValue(&config.store.grndlevel, (uint16_t)atoi(_wsval), true);
Serial.printf("######################### New ground level: %d\n", config.store.grndlevel);			// Контроль
        return;
      }
...
bool NetServer::importPlaylist() {
  ...
  File tempfile = LittleFS.open(TMP_PATH, "r");   // SPIFFS -> LittleFS
  if (!tempfile) {
    return false;
  }
  ...
  if (config.parseCSV(linePl, nsBuf, nsBuf2, sOvol)) {
    tempfile.close();
    LittleFS.rename(TMP_PATH, PLAYLIST_PATH);   // SPIFFS -> LittleFS
    requestOnChange(PLAYLISTSAVED, 0);
    return true;
  }
  if (config.parseJSON(linePl, nsBuf, nsBuf2, sOvol)) {
    File playlistfile = LittleFS.open(PLAYLIST_PATH, "w");   // SPIFFS -> LittleFS
    ...
    playlistfile.close();
    tempfile.close();
    LittleFS.remove(TMP_PATH);   // SPIFFS -> LittleFS
    requestOnChange(PLAYLISTSAVED, 0);
    return true;
  }
  tempfile.close();
  ...
  if(request->url()=="/upload"){
    if (!index) {
      if(filename!="tempwifi.csv"){
        ...
        if(LittleFS.exists(PLAYLIST_PATH)) LittleFS.remove(PLAYLIST_PATH);   // SPIFFS -> LittleFS
      ...
      freeSpace = (float)LittleFS.totalBytes()/100*92-LittleFS.usedBytes();   // SPIFFS -> LittleFS
      request->_tempFile = LittleFS.open(TMP_PATH , "w");   // SPIFFS -> LittleFS
    }else{
    ...
    if (!index) {
      int target = (request->getParam("updatetarget", true)->value() == "littlefs") ? U_SPIFFS : U_FLASH;   // SPIFFS -> LittleFS
...
// Якщо це плейлист або налаштування Wi-Fi, зберігаємо в папку /data/
      ...
      request->_tempFile = LittleFS.open(spath + filename , "w");   // SPIFFS -> LittleFS
    }
    if (len) {
    ...
    if (final) {
      if(request->_tempFile) request->_tempFile.close();   // Безпечне закриття
  ...
  if(network.status == CONNECTED){
    ...
    if(paramsNr==1){
      ...
      if(cmd.exec(p->name().c_str(),p->value().c_str())) {
        if(p->name()=="reset" || p->name()=="clearlittlefs") request->redirect("/");   // SPIFFS -> LittleFS
        if(p->name()=="clearlittlefs") { delay(100); ESP.restart(); }   // SPIFFS -> LittleFS
