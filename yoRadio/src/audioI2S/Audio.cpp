...
/*****************************************************************************************
    ...
    Version 3.4.6g
  Updated on: May 28, 2026, 31.05.2026 (Maleksm)
...
void Audio::setDefaults() {
    ...
    m_streamTitle.reset();
    m_streamURL.reset();    m_resumeFilePos = -1;
...
/*    if(m_fileStartPos > 0 && m_nominal_bitrate){
        if (getBitRate() > 0)
             setAudioFilePosition(m_fileStartPos);
        else
            info(*this, evt_info, "can't set audio play time directly");
            m_fileStartPos = -1;
    }*/
    ...
    exit:
        ...
        if (afn.valid()) { info(*this, evt_eof, "{}", afn.c_get()); /*afn.clear();*/ }
        return;
    }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————-
...
//    constexpr float BALANCE_DB = -16.0f;
    ...
    float reduction = 1.0f - (balance_factor * 0.85f); // ��������� ����� �� 15% ��������� ��������

    if (m_audio_items.balance > 0) { // ������ -> ����� ����
        l_mul = reduction;
    } else if (m_audio_items.balance < 0) { // ����� -> ������ ����
        r_mul = reduction;
    }
