...
#define LFS_MAXLENGTH_FILEPATH 32   // SPIFFS -> LittleFS
...
static void loadExcludeList(fs::FS &_fs, const char *filename){
    static char linebuf[LFS_MAXLENGTH_FILEPATH];   // SPIFFS -> LittleFS
    ...
    if (excludeFile.size() > 0){
      ...
      while (excludeFile.available()){
        ...
        } while ((lastChar >= 0) && (lastChar != '\n') && (idx < LFS_MAXLENGTH_FILEPATH));   // SPIFFS -> LittleFS

        if(isOverflowed){
          isOverflowed = (lastChar != '\n');
          continue;
        }
        isOverflowed = (idx >= LFS_MAXLENGTH_FILEPATH);   // SPIFFS -> LittleFS
        linebuf[idx-1] = '\0';
