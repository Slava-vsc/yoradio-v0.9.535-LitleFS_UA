...
if (this->encoderBPin != 255 && this->encoderBPin != -1) {
    _bVal = digitalRead(this->encoderBPin);
}
if (this->encoderAPin != 255 && this->encoderAPin != -1) {
    _aVal = digitalRead(this->encoderAPin);
}

int8_t ENC_PORT = (_bVal ? (1 << 1) : 0) | (_aVal ? (1 << 0) : 0);
