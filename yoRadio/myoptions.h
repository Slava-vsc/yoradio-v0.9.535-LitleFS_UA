/*******************************************************************************************
ESP32-S3 + ILI9341(SPI2) + I2S + SD(SPI2) + Encoder1. Maleksm мод + батарейка + Mute. Делитель (38.5х99.2) - 1-й или 2-й пин.
********************************************************************************************/
...
//#define USE_BUILTIN_LED   false     /* Use S3-onboard RGB led. По умолчанию "true" */
...
//#define RSSI_DIGIT        true      /* Графический/цифровой значок Wi-Fi. (По умолчанию "false" - лесенка, "true" - цифры) */
...
//#define DSP_MODEL    DSP_ST7796 /* Select ST7796*/
//#define DSP_MODEL    DSP_SSD1306x32    /* Select SSD1306-128х32*/
//#define DSP_MODEL    DSP_SH1106    /* Select SH1106-128х64*/
/* **************************************** */

/* TFT */
#define TFT_CS          10    /* 10-(FSPI CS0) or 39 (SubSPI CS1)   can be CS pin ( ) */
#define TFT_RST         -1    /* SPI RST pin. (-1 if connect to EN) */
#define TFT_DC           9    /* 9-(FSPI HD)  or 33 (SubSPI HD)    can be DC, RS */
#define BRIGHTNESS_PIN  14    /* 14-(FSPI WP)  or 38 (SubSPI WP)    can be BLK, BL - Pin for brightness (output 0 - 3v3) */
...
//#define I2S_MCLK      15    /*  MCLK - Master Clock. (МОД Maleksm с версии v0.9.535(m)-3.4.2p) */
/* **************************************** */

/*  VS1053 PINS. */
#define VS1053_CS     255   /* (to 17) or 39 (SubSPI CS1)    = XCS pin. */
//#define VS1053_DCS    18   /*             = XDCS pin.  */
//#define VS1053_DREQ   16   /*             = DREQ pin.  */
//#define VS1053_RST     8   /* Set to -1 if connected to Esp EN pin, or any other = XRESET pin. */
...
#define SDC_CS          39          /* SDCARD CS pin */
					// Connect SDC_MOSI to  40  /* On board may be D1 pin */
					// Connect SDC_SCK to   41   /* On board may be CLK pin */
					// Connect SDC_MISO to  42  /* On board may be D0 pin */
...
#define MUTE_VAL    LOW   /*  Уровень на MUTE_PIN когда плеер в режиме "Стоп". По умолчанию "HIGH" */
...
//#define ROTATE_90 true   /* Поворот на 90 гр. квадратных дисплеев (и ST7735s, ILI9341, ILI9488, ST7789, ST7796 - МОД Maleksm) . По умолчанию "false"*/
//#define HIDE_VOLPAGE      /* Скрыть страницу "Громкость". (МОД nva_lw и Maleksm)  */
#define FULL_SCR_CLOCK false    /* Скрыть всё кроме часов в скринсейвере. По умолчанию "true" */
//#define HIDE_DATE           /* Скрыть дату. */
//#define BOOMBOX_STYLE     /* Вариант горизонтального"показометра" VUmetr. (По умолчанию Стиль "Studio" - МОД Maleksm)  */
//#define WAKE_PIN              255
//#define CPU_LOAD            /* CPU load widget (МОД Maleksm) */
//#define DOWN_LEVEL           2      /* Уровень глушения (от 0 до 255) По умолчанию "2". WEB. (МОД Maleksm) */
//#define DOWN_INTERVAL     60     /* Интервал BacklightDown в сек (60 сек = 1 мин) По умолчанию "60". WEB. (МОД Maleksm) */
/* ***************************************** */

/*  Аккумулятор (МОД Maleksm) */
//#define BATTERY_ON               /* Включить разметку и показатель уровня заряда и напряжения аккумулятора. WEB. */
//#define HIDE_VOLT                   /* Скрыть только показатель напряжения аккумулятора. WEB. */
...
#define DELTA_BAT    0.056    /* Величина коррекции показаний напряжения батареи в вольтах. WEB. */
/* ***************************************** */
#define GRND_HEIGHT      231     /* (231 м) Высота местности над уровнем моря в метрах для поправки в давление. WEB. */
...
//#define TM_CLK       3    /* GPIO для CLK. Для TM1637 обязательный параметр. */
//#define TM_DIO       8    /* GPIO для DIO. Для TM1637 обязательный параметр. */
